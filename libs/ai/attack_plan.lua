...
function attack_plan.do_target_search(attack_data)
    ...
    value = (value * 1000) / (1 + ((dx * dx) + (dy * dy)))

    if region_data.poor_attack_targets and value > 1 then
...
function attack_plan.tick(attack_data)
    ...
    if attack_data.search then
        ...
        local search = math.min(5 * expansion_phase.compute_time, #attack_data.search.chunks)
        for i = 1, search do
    ...
    if attack_data.unit_group then
        ...
        -- 1 minute
        if attack_data.attack_tick + (60 * 60 * 60 * 1) > game.tick then
            attack_plan.complete_plan(attack_data)
        end
    else
...
function attack_plan.coordinate_biters(attack_data)
    ...
    local range = math.min(35, 15 + (3 * attack_data.biter_base.count)) + math.random(2, 20)
...
function attack_plan.new(surface_name, position, region_key, biter_base)
    return { class = "attack_plan", position = position, surface_name = surface_name, region_key = region_key, biter_base = biter_base, completed = false }
end
